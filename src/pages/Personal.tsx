...
function getEmployeeMonthlyGoal(employee: Employee | null) {
  if (!employee) return 0;
  return Number(employee.monthly_goal) || (isAna(employee) ? ANA_MONTHLY_GOAL : Number(employee.monthly_salary) || 0);
}
...
export default function Personal() {
  ...
  const loadData = async () => {
    try {
      ...
      const normalizedEmployees = employeesData.map(employee => {
        if (isAna(employee) && (!employee.hourly_rate || employee.payment_type !== 'por_hora' || !employee.monthly_goal)) {
          return {
            ...employee,
            hourly_rate: Number(employee.hourly_rate) || ANA_HOURLY_RATE,
            monthly_goal: Number(employee.monthly_goal) || ANA_MONTHLY_GOAL,
            payment_type: 'por_hora' as PaymentType,
            position: employee.position || 'ASISTENTE',
          };
        }
        return employee;
      });
  ...
  const openEditEmployee = (employee: Employee) => {
    ...
    setEmployeeForm({
      ...
      monthly_goal: getEmployeeMonthlyGoal(employee),
  ...
  const monthlyGoal = getEmployeeMonthlyGoal(selectedEmployee);
              ...
              employees.map(emp => (
                ...
                >
                  ...
                  <p className="text-xs text-petrol-400 mt-1">
                    {emp.payment_type === 'por_hora' ? `Objetivo: ${formatCurrency(getEmployeeMonthlyGoal(emp))}` : `Mensual: ${formatCurrency(Number(emp.monthly_salary))}`}
                  </p>
                </button>
              ))
            )}
            ...
            <>
              <div className="bg-crudo-50 dark:bg-slate-800 rounded-xl p-4 border border-petrol-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mt-4 pt-4 border-t border-petrol-100 dark:border-slate-700">
                  <SummaryCard label="Objetivo mensual" value={formatCurrency(monthlyGoal)} />
        ...
        <Modal title={editingEmployee ? 'Editar Empleado' : 'Nuevo Empleado'} onClose={() => setShowEmployeeModal(false)} maxWidth="max-w-md">
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <NumberInput label="Objetivo mensual" value={employeeForm.monthly_goal} onChange={value => setEmployeeForm(f => ({ ...f, monthly_goal: value }))} />
